...
@torch.no_grad()
def evaluate(model, feature, label, split_idx, eval_func, criterion):
    model.eval()
    out = model(feature)
  
    train_acc = eval_func(label[split_idx['train']], out[split_idx['train']])
    valid_acc = eval_func(label[split_idx['valid']], out[split_idx['valid']])
    test_acc = eval_func(label[split_idx['test']], out[split_idx['test']])

    return train_acc, valid_acc, test_acc


@torch.no_grad()
def evaluate_old(model, dataset, split_idx, eval_func, criterion):
    model.eval()
    out = model(dataset)
...
def load_fixed_splits(dataset_name):
    splits_str = 'data/splits/'+dataset_name+'-splits.npy'
    splits_lst = np.load(splits_str, allow_pickle = True)
...
def load_fixed_splits1(dataset, sub_dataset):
